...
{
    char sql[1024] = {0};
    snprintf(sql, sizeof(sql), "insert into groupchat(groupname, groupdesc) values('%s', '%s')", 
...
{   
    // 先根据uerid在groupuser表中查询出该用户所在群组信息
    // 再根据群组信息，查询群组里面所有用户的userid
    // 并和user表进行多表联合查询，查出用户信息
    char sql[1024] = {0};
    ...
    {
        snprintf(sql, sizeof(sql), "select a.id, a.name, a.state, b.grouprole from user a inner join groupuser b on a.id = b.userid where b.groupid = %d", group.getId());
        MYSQL_RES *res = mysql.query(sql);
        if (res != nullptr)
        {
            MYSQL_ROW row;
